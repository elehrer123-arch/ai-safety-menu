...
(function (root) {
  ...
  function cut(ids, D, k) {
    k = k || 'central'; const G = (D.risk && D.risk.groups) || {}, groups = {}; let keep = 1;
    expand(ids, D).forEach(p => { const r = p.risk[k], g = p.risk.group;
  ...
  // "4.95%", "0.376%", "19.8%"
  function fmtP(p) { const v = p * 100; if (v <= 0) return '0%'; return (v >= 10 ? v.toFixed(1) : Number(v.toPrecision(3)).toString()) + '%'; }
  ...
  function best(pool, budget, cost, D) {
    const r1 = p => one(p, D), base = pool.filter(p => cost(p) <= 1e-9 && r1(p) >= 0).map(p => p.id), rest = pool.filter(p => cost(p) > 1e-9 && r1(p) > 0);
    ...
    for (let more = true; more;) { more = false; const v0 = cut(cur, D);
      for (const p of rest) { if (cur.indexOf(p.id) >= 0) continue; for (const out of [null].concat(cur)) { const t = cur.filter(x => x !== out).concat(p.id); if (spent(t) <= budget + 1e-9 && cut(t, D) > v0 + 1e-9) { cur = t; more = true; break; } } if (more) break; } }
    return { cut: cut(cur, D), ids: cur };
  }
